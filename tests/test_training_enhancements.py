...
@pytest.fixture
def train_safe_model_import():
    """Fixture to properly import train_safe_model and clean up sys.path.
    
    This fixture adds the scripts directory to sys.path, yields the imported module,
    ...
    try:
        import train_safe_model
        # Force reimport to get fresh module
        import importlib
        importlib.reload(train_safe_model)
        yield train_safe_model
    finally:
        if scripts_path in sys.path:
            sys.path.remove(scripts_path)


def test_train_safe_model_get_cv_splitter_raises_without_date_column(train_safe_model_import):
    """Test that get_cv_splitter raises MissingDateColumnError without date column."""
    from ml.training import MissingDateColumnError
    
    get_cv_splitter = train_safe_model_import.get_cv_splitter
...
def test_train_safe_model_get_cv_splitter_succeeds_with_date_column(train_safe_model_import):
...
def test_train_safe_model_get_cv_splitter_uses_end_date(train_safe_model_import):
...
def test_train_safe_model_get_cv_splitter_uses_season_column(train_safe_model_import):
...
def test_train_safe_model_no_random_split_fallback(train_safe_model_import):
