...
def normalize_0_100(values):
    """
    Rescale a list of values to 0–100.
...
def analyze_normalization_impact(df):
    ...
    for signal in ['wiki', 'trends', 'youtube', 'spotify']:
        ...
        print(f"  (This would map min→0, max→100)")
...
🔴 BATCH-RELATIVE NORMALIZATION vs. GLOBAL SCORES
...
baselines.csv contains GLOBAL scores:
  ...
  - Min across all titles → 0, Max across all titles → 100
...
OTHER POTENTIAL ISSUES:
  ✓ Precision: baselines.csv has mix of integers and decimals (not an issue)
  ✓ Rounding: Some variation but not systematic (minor)
  ✓ Data sources: Both historical and reference use same methodology (OK)
  ✗ Batch normalization: THIS IS THE MAIN PROBLEM
